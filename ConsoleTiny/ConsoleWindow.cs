		...
		class ConsoleAttachToPlayerState : GeneralConnectionState {
			...
			void PlayerLoggingOptionSelected() {
				PlayerConnectionLogReceiver.instance.State =
					IsConnected() ? PlayerConnectionLogReceiver.ConnectionState.Disconnected : PlayerConnectionLogReceiver.ConnectionState.CleanLog;
			}
			...
			void FullLogOptionSelected() {
				PlayerConnectionLogReceiver.instance.State =
					IsLoggingFullLog() ? PlayerConnectionLogReceiver.ConnectionState.CleanLog : PlayerConnectionLogReceiver.ConnectionState.FullLog;
			}
...
public override void AddItemsToMenu(GenericMenu menu, Rect position) {
		...
		private int RowHeight {
			get { return (Constants.LogStyleLineCount * m_LineHeight) + m_BorderHeight; }
		}

		private static bool HasFlag(ConsoleFlags flags) {
			return (CoreLog.LogEntries.consoleFlags & (int)flags) != 0;
		}

		private static void SetFlag(ConsoleFlags flags, bool val) {
			CoreLog.LogEntries.SetConsoleFlag((int)flags, val);
		}
		...
		void OnGUI() {
			...
			bool setLogFlag = GUILayout.Toggle(LogEntries.wrapped.HasFlag((int)ConsoleFlags.LogLevelLog),
				new GUIContent((logCount <= 999 ? logCount.ToString() : "999+"), logCount > 0 ? iconInfoSmall : iconInfoMono), Constants.MiniButton);
			bool setWarningFlag = GUILayout.Toggle(LogEntries.wrapped.HasFlag((int)ConsoleFlags.LogLevelWarning),
				new GUIContent((warningCount <= 999 ? warningCount.ToString() : "999+"), warningCount > 0 ? iconWarnSmall : iconWarnMono), Constants.MiniButton);
			bool setErrorFlag = GUILayout.Toggle(LogEntries.wrapped.HasFlag((int)ConsoleFlags.LogLevelError),
				new GUIContent((errorCount <= 999 ? errorCount.ToString() : "999+"), errorCount > 0 ? iconErrorSmall : iconErrorMono), Constants.MiniButton);
			...
			if (m_StacktraceLineContextClickRow != -1) {
				...
				if (LogEntries.wrapped.StacktraceListView_CanOpen(stacktraceLineInfoIndex)) {
					...
					if (LogEntries.wrapped.StacktraceListView_CanWrapper(stacktraceLineInfoIndex)) {
						menu.AddItem(new GUIContent("Wrapper"), LogEntries.wrapped.StacktraceListView_IsWrapper(stacktraceLineInfoIndex),
							LogEntries.wrapped.StacktraceListView_Wrapper, stacktraceLineInfoIndex);
					}
				}
		...
		private void AddStackTraceLoggingMenu(GenericMenu menu) {
			...
			foreach (LogType logType in Enum.GetValues(typeof(LogType))) {
				foreach (StackTraceLogType stackTraceLogType in Enum.GetValues(typeof(StackTraceLogType))) {
					...
					menu.AddItem(EditorGUIUtility.TextContent("Stack Trace Logging/" + logType + "/" + stackTraceLogType),
